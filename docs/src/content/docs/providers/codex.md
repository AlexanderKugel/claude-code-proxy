...
`CCP_CODEX_RESPONSES_API=1` enables both `POST /v1/responses` and `POST /v1/chat/completions`. The route gate lives under Codex configuration, while model routing on both endpoints also supports Kimi, Grok, and Cursor Agent.
